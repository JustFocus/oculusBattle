(function() {
	...
	Game.prototype.aiMove = function (delta) {
		this.timer += delta;
		this.aiTimer += delta;
		if (this.aiTimer > (Math.random() * 4000 + (8000 / this.ai.power))) {
		...
		for (var i = 0; i < this.bases.length; i++) {
			if (this.bases[i].team !== 'neutral' &&
				...
				if (this.bases[i].team === 'cpu'){
					levelMult = ((this.ai.power - 1) * 0.10) + 1;
				} else {
					levelMult = 1;
				}
	...
	Game.prototype.generateBases = function () {
		...
		var bases = [
			{id: 1, xCoord: 75, yCoord: 250, team: 'player', value: 30, startValue: 30},
			{id: 2, xCoord: 725, yCoord: 250, team: 'cpu', value: 30, startValue: 30}
		];
		var baseCount;
		var verticalSplit;
		var padding = 10;
